...
const App = () => {
  // State for expenses, using localStorage hook for persistence
  ...
  const handleAddExpense = useCallback(() => {
    ...
    setMessage('Success! Expense added.');
  ...
  // Delete a single expense handler
  const handleDeleteExpense = useCallback((id) => {
    setExpenses(prev => prev.filter(expense => expense.id !== id));
    setMessage('Expense deleted.');
  }, [setExpenses]);

  // Tailwind CSS classes for the gradient background
      ...
      <div className="w-full max-w-lg bg-white bg-opacity-95 backdrop-blur-sm rounded-xl shadow-2xl p-6 space-y-6">
        ...
        <div className="p-4 rounded-xl bg-indigo-500 shadow-lg mt-6">
          ...
          <p className="text-sm text-indigo-200 mt-1">
            All current expenses tracked in this session.
          </p>
        </div>
